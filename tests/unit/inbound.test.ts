...
describe('InboundService（v2.5.7 协议增量 E2：只读域投影 listSince / get）', () => {
  it('listSince：无 since → 全量；with since → updated_at 严大于过滤；非法 since → 全量', async () => {
    const home = await tmp()
    const ws = await tmp()
    const { workspace, inbound } = build(home)
    await workspace.create(ws)
    const src = await makeSourceFile()
    const rel = await inbound.archiveFile(src, '2026/08/11')
    await inbound.create({ ...baseReq({ id: 'SINCE-2' }), file_path: rel })
    expect(await inbound.listSince()).toHaveLength(1)
    expect(await inbound.listSince('2016-01-01T00:00:00.000Z')).toHaveLength(1)
    const all = await inbound.list()
    expect(await inbound.listSince(all[0].updated_at)).toHaveLength(0)
    expect(await inbound.listSince('garbage')).toHaveLength(1)
  })

  it('get：按单据编号查重主键；命中返回记录、未命中 null（与 checkId 同源）', async () => {
    const home = await tmp()
    const ws = await tmp()
    const { workspace, inbound } = build(home)
    await workspace.create(ws)
    const src = await makeSourceFile()
    const rel = await inbound.archiveFile(src, '2026/08/11')
    await inbound.create({ ...baseReq({ id: 'GET-88' }), file_path: rel })
    expect((await inbound.get('GET-88'))?.id).toBe('GET-88')
    expect(await inbound.get('NOPE')).toBeNull()
  })
})
