 ...
 * 白屏像素检测的唯一真相源：判定截屏是否为「接近加载底色 / 纯白」的空窗。
 *
 * 设计要点（评审后收紧）：
...
/** 显示后白屏自检的扩展判定色：浅色 + 深蓝空窗（覆盖真机「蓝白屏」两种残留） */
...
/** 截屏是否为「空白空窗」（接近加载底色/纯白，或扩展判定色）——是则白屏自检应继续升级。
 *  targets 默认浅色系（保持既有语义）；自检复检传 BLANK_TARGETS_WAKE（含深蓝空窗）。
