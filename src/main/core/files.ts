...
import type { ListSubfoldersRequest, SubfolderEntry } from '../../shared/types'
...
export class FilesService {
   ...
   */
  /**
   * v2.5.9（A9 刀1）：某实体某域下**实际存在的子文件夹**——上方那排 tab 名单的唯一来源。
   *
   * 为什么要换掉"读全局 config 表"：那张表全站一份，于是
   *  ① 盘上/坚果云同步进来的目录表里没登记 ⇒ **存在却看不见**（P2）；
   *  ② 在 A 集删一个类型会把名字从表里划掉 ⇒ **B 集的 tab 也跟着没**（P1，文件其实好好的）。
   * 口径（`docs/INTERNAL/DESIGN-v2.5.9-A9-子文件夹以盘为准.md` §二 §三）：
   *  - 空目录**照样列出**，用 `has_files` 让渲染层淡一档显示（用户拍板"空的要显示并且淡一点"）；
   *  - **排序只在这里做**（§八 实测：readdir 原序稳定但不是名称序，且不保证跨平台一致）；
   *  - 符号链接目录不列（`isDirectory()` 对 symlink 为假）——工作区结构是应用自己建的，
   *    列进来反而会把外链算成 tab；真需要支持时按 A9 的另一次拍板处理。
   */
  async listSubfolders(req: ListSubfoldersRequest): Promise<SubfolderEntry[]> {
    const ws = this.requireWS()
    const holder = assertSafePathSegment(
      req.product_set,
      req.scope === 'customer' ? '客户名称' : req.scope === 'supplier' ? '供应商名称' : '产品集',
    )
    const parent =
      req.scope === 'customer'
        ? path.join(ws, CUSTOMERS_DIR, holder)
        : req.scope === 'supplier'
          ? path.join(ws, SUPPLIERS_DIR, holder)
          : path.join(
              ws,
              PRODUCT_SETS_DIR,
              holder,
