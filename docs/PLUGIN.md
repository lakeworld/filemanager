...
export interface PluginCatalogEntry {
  ...
  images?: string[]     // 截图 URL（https；v2.6.2 增量，宿主读侧最多收 3 张、坏项逐条丢，全坏则整字段缺省）。**在途：2.6.1 / 2.6.2 未发布**，宿主能力探测为准；发布前以本句为时效标记
  detail?: string       // 功能详情长文（纯文本可含换行；v2.6.2 增量。长度上限在服务端写入侧把关，宿主不截断）。**在途：2.6.1 / 2.6.2 未发布**，宿主能力探测为准；发布前以本句为时效标记
...
export interface PluginCatalogVersion {
  ...
  releaseNotes?: string              // 本版更新说明（v2.6.2 增量；空串/纯空白按缺省处理，界面如实说"官方没写"而不是藏掉）。**在途：2.6.1 / 2.6.2 未发布**，宿主能力探测为准；发布前以本句为时效标记
