...
- **权益标记**：官方插件可在 manifest 声明 `entitlement` 取钥门槛（字段语义见 §三）；宿主只做门槛校验，不实现任何计费逻辑。
