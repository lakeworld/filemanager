...
- 测量口径：`performance.now()` 打点 + 路由专属 H1 可见即 ready，每路由连续 5 次 reload 采样、判 **p90（升序第 4 值）**（旧口径 3 样本中位数的容差带只有 7–8ms、小于一帧，单峰分布下最大样本落带外概率就是两位数；`Date.now()` 与粗糙文本长度判据更早已废）。
- 双层判定：**3000ms 灾难线**（任何机器必须过）＋ **冻结基线 p90 回归门禁**（同机 `os.platform()` 可比时，p90 超过 `max(基线×1.25×1.15, 基线+20ms)` 即失败，迫使在动作文档记录并解释）。**基线数值不动**（重冻结属阈值纪律）；CI 与本地同一套代码，平台不可比退化为灾难线并显式告警（旧版 `if (process.env.CI)` 特判正是"CI 绿但本地红"的机制来源）。
