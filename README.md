...
npm test             # 单元测试（vitest，仅 tests/unit，245 用例）
npm run test:e2e     # 端到端测试（Playwright，43 用例，e2e 模式 QIHEBOX_E2E=1）
...
| 平台 | 产物 | 目标体积 |
|---|---|---|
| Linux | `release/启禾文件管理-2.4.8.AppImage` / `qihe-box_2.4.8_amd64.deb` | ≤ 140MB |
| Windows | `release/启禾文件管理-2.4.8-win-x64.zip`（便携） / `Setup 2.4.8.exe`（NSIS） | ≤ 195MB |
