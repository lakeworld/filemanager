...
> **启禾生态成员** · 云端企业级能力见 [启禾 OS](https://www.qihebook.cloud/)——AI 原生 ERP，自然语言操作（订单 / 库存 / 财务 / 客户）
...
>
> 授权模式：**本体开源免费**。本地文件管理核心（索引、预览、产品集、命名模板等）永久免费，无需激活，下载即用全部核心功能。开源与收费承诺详见 [HELP.md](HELP.md) 十八。

## 产品定位

**启禾文件管理** 是一款面向 **电商运营、销售团队以及线下客户经理** 的本地化产品资料管理工具。

> **与启禾 OS 的关系**：文件管理器是独立于启禾 OS 的开源免费桌面产品，不依赖启禾 OS 账号即可使用；但它延续启禾 OS 的品牌、设计语言与工程标准，可视为启禾产品生态向本地工具场景的延伸（BOX 项目）。
...
- XLSX 模板导入/导出（批量建产品集）
- **标签体系**：父/子标签层级，颜色可自由修改，删除后不再复活（不再有固定预设标签），文件与产品集统一打标；v2.4.4 起：**搜索命中标签、文件卡片标签展示、证书库/图包库/文件浏览器标签筛选、多选批量打标**，打标时输入未定义名即建新标签（「父/子」语法建子标签），标签管理页支持「移至父级」（引用源注册机制已服务 v2.4.7 客户/发票台账，rename/delete/计数自动覆盖）
...
- 证书到期提醒（30 天内）
- **客户管理（v2.4.7）**：客户目录 + `customers.json` 档案（别名/国家/联系方式/来源/标签/备注），默认子文件夹（报价/合同/沟通/其他，可管理），列表/详情双态页 + 子文件夹文件区，客户 ↔ 产品集关联互跳；删除进回收站可恢复
- **发票台账与入库归档（v2.4.7）**：发票号码查重三入口拦截、状态流转（待报销/已报销/已入账）、30 天待办提醒（仪表盘区块 + 系统通知合并）、Excel 导出、文件归档 `发票/<YYYY>/`；入库单 6 字段 + 单据编号查重 + 归档 `入库/<YYYY>/`
- **交换区投递（v2.4.7）**：`工作区/交换区/` 手工或系统投递（文件 + 描述 JSON）→ 归集 + 回执，幂等簿记与崩溃恢复，插件未启用也可用
- **打磨轮（v2.4.8）**：视频进度条修复（moov 尾部大视频拖动进度条失效）、休眠唤醒白屏自愈、反馈体系统一（应用内提示 + 统一确认框）、设置页标签树折叠、一键打包（整图包/当前筛选证书）、弹窗 Esc、窗口状态记忆、缩略图缓存自动清理；同日补丁轮：右键菜单根治、侧边栏导出区（/exports）、休眠唤醒加固、客户拖放区适配
- **供应商管理（v2.4.9）**：供应商台账 + `供应商/<名>/` 目录（合同/对账单/往来文件）+ 列表/详情页 + 入库单供应商下拉关联 + 关联产品集（供应商侧写、产品集侧只读反查）
- **报价单（v2.4.9）**：台账对齐启禾 OS 报价单 Quotation（明细行 + 三态状态机）+ `报价/<YYYY>/` 归档 + 单号自动生成 + 客户详情报价联动 + 列表筛选
- **开机自启（v2.4.9）**：设置页开关；`--autostart` 延迟建窗托盘常驻；托盘打开固定回仪表盘（冷启动仍恢复上次页面）
- **客户对齐启禾 OS（v2.4.9）**：客户新增 类型/电话/邮箱/地址 + `erp_ext` 预留命名空间（为 v2.7 erp-bridge 铺路）
- **命名模板编号槽位（v2.4.9）**：`sequence` 槽位（导入批次编号）+ 批量重命名复用模板
- **日志系统（v2.4.9）**：主进程日志 core 化 + 容量上限自动清理 + 渲染错误转发 + 「我的」页日志卡片（导出）
- **插件系统（v2.5）**：公开插件协议（API_VERSION=1，发布即冻结只增不删）+ 三段插件宿主 + `.qbox` 侧载安装（设置 → 插件 → 开发者模式，风险确认框）+ hello 开源教学插件（`src/plugins/hello/README.md` 15 分钟上手）；协议测试四件套（API 兼容性守护 / 契约对账 / 模糊测试 / `npm run conformance` 一致性体检）守护「协议即承诺」；普通用户零变化（官方插件索引 v2.6+ 上线）
- 全局搜索（异步 + 防抖 + 扫描缓存）
...
| 平台 | 产物 | 目标体积 |
|---|---|---|
| Linux | `release/启禾文件管理-2.4.9.AppImage` / `qihe-box_2.4.9_amd64.deb` | ≤ 140MB |
| Windows | `release/启禾文件管理-2.4.9-win-x64.zip`（便携） / `Setup 2.4.9.exe`（NSIS） | ≤ 195MB |
