...
[![Version](https://img.shields.io/badge/Version-v2.6.2-green.svg)]()
...
- **回收站兜底**：删除先进回收站，30 天内可恢复（到期条目在应用启动时自动清理），标签、备注、缩略图原样找回
...
- **回收站**：删除的内容先进回收站，保留 30 天（到期条目在应用启动时自动清理，每条显示剩余可恢复天数）；可恢复（回到原位置，标签备注原样）或彻底删除
...
- **不怕误删**：删除进回收站，30 天内可恢复（到期条目在应用启动时自动清理），回收站里每条都写着剩余天数
