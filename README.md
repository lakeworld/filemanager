...
- XLSX 模板导入/导出（批量建产品集）
- **标签体系**：父/子标签层级，颜色可自由修改，删除后不再复活（不再有固定预设标签），文件与产品集统一打标；v2.4.4 起：**搜索命中标签、文件卡片标签展示、证书库/图包库/文件浏览器标签筛选、多选批量打标**，打标时输入未定义名即建新标签（「父/子」语法建子标签），标签管理页支持「移至父级」（引用源注册机制为 v2.6 客户 / v2.7 发票复用铺路）
- **临时压缩分享**：多选文件 → 右键「压缩分享」，流式打包到工作区 `导出/`（零新增依赖、恒定内存），完成后一键打开所在文件夹 / 复制到剪贴板外发
- **zip 解压**：右键 `.zip` → 解压到当前文件夹 / `<包名>/`；zip-slip 路径逃逸防护、GBK 文件名解码、同名自动加序号
- **视频浏览**：视频帧缩略图（渲染层 canvas 抓帧，不依赖 ffmpeg）+ 图包库图片/视频聚合筛选
- 证书到期提醒（30 天内）
- 全局搜索（异步 + 防抖 + 扫描缓存）
...
环境要求：Node.js 20+（Electron 31 运行时，v2.4.5 起）
