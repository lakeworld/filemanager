...
- **PDF 预览全面重做（打包版曾空白/报错/闪退/极慢）**：
  - 改为**独立预览窗口**（独立渲染进程）：点开临时进程、关闭即销毁释放内存，主窗口常驻内存大幅下降
  - 流式加载（Range 请求，不再全量读内存）+ 页缓存 LRU + 降采样渐进渲染 + 渲染队列（快速翻页不白屏）+ worker 单例 + 组件销毁清理
- **图片预览窗口**：全分辨率大图 + 下方同目录缩略图条，点击切换
...
- 目标：常驻内存 ~400MB 内稳定（去 GPU + 懒加载实测），长期运行不增长
